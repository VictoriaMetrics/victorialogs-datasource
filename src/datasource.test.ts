...
import { AdHocFiltersMode, Query, QueryType, SupportingQueryType } from './types';
...
describe('VictoriaLogsDatasource', () => {
  ...
  describe('applyTemplateVariables', () => {
    ...
    it('should preserve existing extraFilters and apply them to root query in rootQuery mode', () => {
      const adhocFilters: AdHocVariableFilter[] = [
        { key: 'level', operator: '=', value: 'error' },
      ];
      const templateSrvMock = {
      ...
      const ds = createDatasource(templateSrvMock);
      const replacedQuery = ds.applyTemplateVariables(
        { expr: '_time:5m', refId: 'A', extraFilters: 'app:="frontend"', adHocFiltersMode: AdHocFiltersMode.RootQuery },
        {},
        adhocFilters
      );
