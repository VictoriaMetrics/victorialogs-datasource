...
export const getMenuGroups = (allowedCategories?: string[]): MenuGroup[] => {
  const groups: MenuGroup[] = [
    ...
    { label: 'Modify', keywords: ['modify', 'transform', 'rename', 'copy', 'extract', 'pack', 'unpack', 'format'], items: MODIFY_TEMPLATES.filter((t) => !allowedCategories || allowedCategories.includes(t.stepCategory)).map(pick) },
    { label: 'Aggregate', keywords: ['aggregate', 'stats', 'count', 'sum', 'avg', 'group'], items: AGGREGATE_TEMPLATES.filter((t) => !allowedCategories || allowedCategories.includes(t.stepCategory)).map(pick) },
    ...
    { label: 'Limit', keywords: ['limit', 'top', 'first', 'last', 'offset'], items: LIMIT_TEMPLATES.filter((t) => !allowedCategories || allowedCategories.includes(t.stepCategory)).map(pick) },
