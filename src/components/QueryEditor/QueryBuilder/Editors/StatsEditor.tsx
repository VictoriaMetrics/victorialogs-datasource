...
export default function StatsEditor(type: "stats" | "running" | "total" = "stats") {
  function StatsEditor(props: QueryBuilderOperationParamEditorProps) {
    ...
    */
    ...
    const queryStatsModeller = useMemo(() => {
      if (type === "stats") {
        ...
        const operations = new OperationDefinitions().getRunningStatsOperationDefinitions();
        return new QueryModeller(operations);
      }
      const operations = new OperationDefinitions().getTotalStatsOperationDefinitions();
      return new QueryModeller(operations);
    }, []);
