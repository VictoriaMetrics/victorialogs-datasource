...
/** A factory whose start is observable: `started` counts the subscriptions and `subject` drives the completion */
const makeTask = () => {
...
describe('QueryScheduler', () => {
  ...
  it('does not release the slot twice when unsubscribed after completion', () => {
    ...
    a.subject.complete();
    // a's slot already moved to b, so this must not free a second slot for c
    subA.unsubscribe();
