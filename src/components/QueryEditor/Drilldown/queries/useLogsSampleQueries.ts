...
/** Runs a raw-logs sample query for the logs carrying one stream field; idle until enabled */
export function useFieldLogsSample(
  ...
  const [data, setData] = useState<PanelData>({ series: [], state: LoadingState.NotStarted, timeRange: range });
  // tracks the field so a genuine identity change resets the sample immediately, while a
  // context-only refetch (range changed, same field) keeps the previous series visible (dimmed)
  ...
  useEffect(() => {
    ...
    const identityChanged = previousFieldRef.current !== field;
    previousFieldRef.current = field;

    setData((prev) =>
      identityChanged ? { series: [], state: LoadingState.Loading, timeRange: range } : { ...prev, state: LoadingState.Loading }
    );
    const target = buildFieldPresenceLogsQuery(query, field, refIdSuffix);
    const request = buildDrilldownRequest([target], range, `drilldown-field-logs-${refIdSuffix}`);
    const subscription = subscribeLogsSample(datasource, request, range, setData);
    return () => subscription.unsubscribe();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [datasource, enabled, query.expr, field, refIdSuffix, range.from.valueOf(), range.to.valueOf()]);
