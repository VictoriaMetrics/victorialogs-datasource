...
import { useFacets, useStreamFields } from '../queries/useFieldListQueries';
...
export const ValueDetails: React.FC<ValueDetailsProps> = ({
  ...
  const streams = useStreamFields(datasource, range, lookupQuery);
  const streamFieldNames = useMemo(() => streams.streamFields.map((f) => f.value), [streams.streamFields]);
  // the Streams tab lists the stream fields directly — facets only feed the Fields tab cards
  const facetsEnabled = activeTab === 'fields';
  ...
  const { otherFacets, otherFallback } = useMemo(() => {
  ...
  const counterSuffixes = useMemo(
    () => ({
      ...
      streams: makeCounterSuffix(streams.streamFields.length),
      fields: makeCounterSuffix(otherFallback.length),
      patterns: makeCounterSuffix(patterns.totalPatterns),
    }),
    [logsCount, streams.streamFields.length, otherFallback.length, patterns.totalPatterns]
  );

  const renderBreakdownTab = (tabFacets: typeof otherFacets, fallbackFieldNames: string[]) => {
    if (drillField) {
      return (
