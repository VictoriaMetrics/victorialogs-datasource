...
/** Volume-chart config for a row with no custom styling. The frames carry their own per-field configs, so only the unit is forced */
...
interface BreakdownRowProps {
  ...
  /** Caption of the volume chart. The logs panel reuses it as `${panelTitle} logs` */
  panelTitle: string;
  ...
  /** Show a per-series legend table on the volume chart, for example the per-level totals */
...
/** One breakdown row: a header with the title and actions, a volume chart, and a logs sample that loads when the row scrolls into view */
export const BreakdownRow: React.FC<BreakdownRowProps> = ({
  ...
  // every number renders in Grafana's compact 'short' form, for example 12.3K. A custom chart
  // config, such as the patterns bar styling, gets the unit merged in rather than replaced
  ...
  // makes the legend interactive when it is shown: a click isolates a series, ctrl or cmd click adds one
...
const getStyles = (theme: GrafanaTheme2) => ({
  ...
  // dims the logs panel while a refetch runs, instead of unmounting it
  refetching: css({
    opacity: 0.6,
