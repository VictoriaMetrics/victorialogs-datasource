...
// a managed IntersectionObserver that collects the callbacks, so a test can trigger them
...
const makeDatasource = () =>
  ({
    // a non-empty logs sample by default. An empty one renders the "No data" placeholder rather
    // than the logs PanelRenderer, and several tests below assert on that renderer
...
/** Marks every collected IntersectionObserver as intersecting, as if the rows scrolled into view */
...
describe('FieldValuesBreakdown', () => {
  ...
  it('renders the volume chart and defers the logs query until the row is visible', async () => {
    ...
    // the logs sample is lazy, so it must not fire before the row becomes visible
  ...
  it('keeps rendering the rows (instead of the placeholder) while loading if previous values are still present', () => {
    ...
    // a refetch of the same field keeps `top` populated, so the rows must stay mounted and
    // dimmed rather than turning into the placeholder
