 ...
 * service-like field, plus a two-level Stream fields tab, more addable via "+"),
 * and a details view (Logs / Streams / Fields / Patterns tabs) shown while a
 * `field = value` filter for the current breakdown field is present ("Filter for" on a
 * value row drills in). Removing that filter chip returns to the main view
 */
const DrilldownDrawer: React.FC<DrilldownDrawerProps> = ({
  ...
  // level-2 state of the Stream fields tab: the stream field whose per-value breakdown is open
  ...
  // field plus the Stream fields tab; user-managed tabs win afterwards
  useEffect(() => {
  ...
  // the field whose `field = value` filter opens the details view: the active breakdown tab,
  // except on the Stream fields tab where it's the drilled stream field (the tab sentinel
  // itself never carries value filters)
  const detailsField = activeTab === STREAM_FIELD ? streamDrillField : activeTab;
