...
// the in-place editor mounts useFieldFetch, which resolves template variables through the
// runtime singleton, and a unit test never initializes that
...
/** Datasource with working lookups, needed by the tests that open the in-place editor */
...
// default props for the "+ Filter" control. Most tests below only need it to render
...
describe('DrilldownFiltersRow', () => {
  it('renders the local filters as segmented chips without the editor chrome', () => {
    ...
    // bare chips only, with no "Ad-hoc filters:" caption and no move-to-query control
  ...
  it('renders chips before the "+ Filter" control', () => {
    ...
    // DOCUMENT_POSITION_FOLLOWING means the add button comes after the chip in DOM order
