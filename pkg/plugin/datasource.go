...
func newDatasourceInstance(ctx context.Context, settings backend.DataSourceInstanceSettings) (instancemgmt.Instance, error) {
	...
	if err := copyDatasourceSettings(settings, &dstSettings); err != nil {
...
func copyDatasourceSettings(settings backend.DataSourceInstanceSettings, dstSettings *DataSourceInstanceSettings) error {
	var settingsJson []byte
	var err error
	if settingsJson, err = json.Marshal(settings); err != nil {
		return fmt.Errorf("failed to encode datasource settings : %w", err)
	}

	if err := json.Unmarshal(settingsJson, &dstSettings); err != nil {
		return fmt.Errorf("failed to parse datasource settings: %w", err)
	}

	if err := json.Unmarshal(settings.JSONData, &dstSettings); err != nil {
		return fmt.Errorf("failed to parse datasource JSONData settings: %w", err)
	}
