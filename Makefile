...
vl-plugin-build-local: vl-frontend-plugin-build app-via-docker-local
	rm -rf plugins/vl-datasource && \
	cp -r plugins/frontend plugins/vl-datasource && \
	cp plugins/backend/$(APP_NAME)_* plugins/vl-datasource/

vl-plugin-build: vl-frontend-plugin-build vl-backend-plugin-build

vl-plugin-pack-zip: vl-plugin-build
	mkdir -p dist && \
	$(foreach plugin,$(wildcard plugins/backend/$(APP_NAME)_*),\
		$(eval SUFFIX := $(subst $(APP_NAME)_,,$(notdir $(plugin)))) \
		$(eval PACKAGE_NAME := victorialogs-datasource-$(PKG_TAG).$(SUFFIX).zip) \
		rm -rf plugins/vl-datasource && \
		cp -r plugins/frontend plugins/vl-datasource && \
		cp $(plugin) plugins/vl-datasource/ && \
		cd plugins/vl-datasource && \
